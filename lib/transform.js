...
exports.transformTransactionsToUpdates = function(transactions) {
   ...
   */
    ...
    // filter BoA Travel Rewards payments
    transactions = _.reject(transactions, function(el){return (id_dict[el.account_id] === 'BoA Travel Rewards' &&
            el.name === 'PAYMENT - THANK YOU'); })
    // filter BoA Cash Rewards
    transactions = _.reject(transactions, function(el){return (id_dict[el.account_id] === 'BoA Cash Rewards' &&
            el.name === 'PAYMENT - THANK YOU'); })
